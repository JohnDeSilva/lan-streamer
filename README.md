...
*   **🗺️ Interactive Manual Episode Mapper**: A fallback mapping tab in the **Series Details** dialog allows pulling episode lists by TMDB Group and subgroup (Story Arc, DVD Order, Cour) and manually mapping local files to them. Users can also designate a specific TMDB Group as the default group for all future scans and metadata updates, persisting the association directly in the library database.
