...
### 📁 File Management
*   **⚡ Library Scanning & Cleanup**: Combined "Scan Library" action that scans for new files and automatically updates paths for moved or missing files in a single pass. Configuration loads dynamically on demand, and adding paths in Settings auto-triggers scans. Displays a detailed, real-time scan progress dashboard. Periodic background scans automatically target only active roots, skipping archive roots to conserve bandwidth and prevent disk thrashing.
