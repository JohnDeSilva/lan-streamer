...
*   **📁 Multi-Library & Per-Root Archive Modes**: Organize content into multiple libraries with support for multiple root directories per library. Individual root paths can be toggled between **Active** (frequently changing, scanned on routine and combined view scans) and **Archive** (static/rarely changing, scanned only via **Full Scan** to prevent network bottlenecks and disk thrashing).
