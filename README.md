...
### 🔍 Metadata Management
*   **🔍 Metadata Matching**: Multi-stage search strategy to link local media to TMDB and Jellyfin entries. Includes metadata locking capabilities to prevent automatic updates during library scans, alongside targeted metadata refresh controls in details windows to manually refresh individual series or episodes. When new episode files are discovered during a scan, the affected series is automatically refreshed from TMDB — including re-pulling names, air dates, and runtimes for already-indexed episodes (their watched state, file versions, and Jellyfin/MyAnimeList mappings are preserved).
...
### 📁 File Management
*   **⚡ High-Performance Library Scanning & Cleanup**: Combined "Scan Library" action that scans active roots for new files and automatically updates paths for moved or missing files in a single pass. A dedicated **Full Scan** action scans across both active and archive directories. Features true directory `mtime` fast-bypass caching to skip individual file `stat` calls over SMB/NFS networks for unchanged seasons and movies, bypasses deep recursive layout checks on unchanged series, and supports configurable scanner thread concurrency (default 4 threads) to prevent mechanical HDD thrashing and network congestion. Periodic background scans and Combined View scans automatically target active roots, skipping archive roots.
...
*   **📑 Library Tabs & Multi-Library Consolidation**: Group any number of local and remote libraries into custom tabs (configured via the **Library Tabs** settings tab). Series or movies spanning multiple libraries within the same tab are seamlessly consolidated into unified records with combined seasons, episodes, and media file versions. Marking items watched propagates across all origin libraries.
*   **⚙️ Settings-Only Per-Library Scanning**: Scanning operations are centralized exclusively in the Settings dialog on a per-library basis. Target libraries can be selected under **Library Management** for full scans, pass-specific scans (File Discovery, Metadata Resolution, Technical ffprobe), and missing-file cleanup, or triggered directly from **Local Libraries Setup** and **Remote Libraries Setup**.
*   **📁 Multi-Library & Per-Library Archive Modes**: Organize content into multiple distinct libraries, each mapped to a dedicated root directory. Individual libraries can be toggled between **Active** (frequently changing, scanned on routine and combined view scans) and **Archive** (static/rarely changing, scanned only via **Full Scan** to prevent network bottlenecks and disk thrashing).
