...
2. **Metadata Resolution (Pass 2)**: Resolves series, season, episode, and movie metadata from TMDb. The resolution logic is implemented in the [services/](file:///home/sadmin/antigravity/lan-streamer/src/lan_streamer/services) package ([metadata_tv.py](file:///home/sadmin/antigravity/lan-streamer/src/lan_streamer/services/metadata_tv.py) for TV, [metadata_movie.py](file:///home/sadmin/antigravity/lan-streamer/src/lan_streamer/services/metadata_movie.py) for movies) and invoked by the scanner orchestration layer ([scanner/core.py](file:///home/sadmin/antigravity/lan-streamer/src/lan_streamer/scanner/core.py)). When executed as part of the pipeline (or individually), it runs with `metadata_only=True` to read files directly from the in-memory output of Pass 1, skipping physical disk checks and asset directory walks.
