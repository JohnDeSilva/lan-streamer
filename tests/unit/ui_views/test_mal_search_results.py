...
class TestExistingMappedIds:
    ...
    def test_mapped_entry_shows_symbol(self, qtbot) -> None:
        results = [_make_result(42, "Mapped Anime")]
        dialog = MalSearchResultsDialog(results=results, existing_mapped_ids={42})
        qtbot.addWidget(dialog)
        title_text = dialog._results_table.item(0, 1).text()
        assert title_text == "\u25cf Mapped Anime"
    ...
    def test_unmapped_entry_no_symbol(self, qtbot) -> None:
        ...
        dialog = MalSearchResultsDialog(results=results, existing_mapped_ids={99})
        qtbot.addWidget(dialog)
    ...
    def test_empty_existing_ids_no_symbol(self, qtbot) -> None:
        ...
        dialog = MalSearchResultsDialog(results=results, existing_mapped_ids=set())
        qtbot.addWidget(dialog)
