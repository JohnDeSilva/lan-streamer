"""
TMDB metadata resolution functions for movies and TV series.

This module provides the core metadata resolution logic extracted from the
scanner layer. It handles building metadata stubs, resolving Jellyfin IDs,
downloading posters, applying TMDB data, and processing series/seasons/episodes
all the way from raw scanner input to fully-resolved metadata dictionaries.

Typical usage is indirect — the public scanner entry points (``scan_movie``,
``scan_series``) delegate to these helpers — but the functions can also be
called independently when fine-grained control over metadata resolution is
needed.
"""
...
logger = logging.getLogger("lan_streamer.services.metadata_resolution")
