...
*   **📁 Multi-Library & Per-Root Archive Modes**: Organize content into multiple libraries with support for multiple root directories per library. Individual root paths can be toggled between **Active** (frequently changing, scanned on periodic schedules) and **Archive** (static/rarely changing, skipped during routine auto-scans to prevent network bottlenecks and disk thrashing).
